@if (!empty($aLists))
     @foreach ($aLists as $aList)
          <tr id="tr_{{$aList->id}}">
            <td>
               <a href="{{ url('images/banners/'.$aList->image) }}" target="_blank"><img src="{{ asset('images/banners/'.$aList->image) }}"></a>
            </td>
            <td>{{ !empty($aList->short_description) && Str::length($aList->short_description) > 60 ? Str::substr($aList->short_description, 0, 60).'...' : $aList->short_description}}</td>
            <td>
               <form id="changeStatus{{$aList->id}}" method="post" onsubmit="return ajax_change_status('changeStatus{{$aList->id}}','{{url('admin/changeStatus')}}','',{{$aList->id}})">
                 @csrf
                 <input type="hidden" name="table" id="table{{$aList->id}}" value="banners">
            ...
            <td>
               <div class="row">
                ...
                ="icons-list" id="delete{{$aList->id}}" method="post" onsubmit="return ajax_delete_record('delete{{$aList->id}}','{{url('admin/delete')}}','',{{$aList->id}})">
                 ...
                 <div class="col-md-6">
                  <form  class
                  ="icons-list" id="edit{{$aList->id}}" method="post" action="{{url('admin/edit-banner/'.$aList->id.'')}}">
                      @csrf
                      <input type="hidden" name="table" id="table{{$aList->id}}" value="banners">
                      <input type="hidden" name="id" id="id{{$aList->id}}" value="{{$aList->id}}">
                      <div class="col-md-12">
